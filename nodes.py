...
class FishSpeech_INFER_SRT:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "text2semantic_type": (["medium", "large"], {"default": "medium"}),
                "hf_token": ("STRING", {"default": "your token to download weights"}),
                "num_samples": ("INT", {"default": 1}),
                "max_new_tokens": ("INT", {"default": 0}),
                ...
                "iterative_prompt": ("BOOLEAN", {"default": True}),
                "max_length": ("INT", {"default": 2048}),
                "chunk_length": ("INT", {"default": 30}),
            }
        }
    ...
    def get_tts_wav(
        ...
        if not os.path.isfile(vq_model_path):
            hf_hub_download(
                repo_id="fishaudio/fish-speech-1.5",
                filename="vq-gan-group-fsq-2x1024.pth",
                local_dir=checkpoint_path,
                token=hf_token,
            )
        ...
        for i, (prompt_sub, text_sub) in enumerate(
            zip(prompt_subtitles, list(SrtPare(text_file_content)))
        ):
            start_time = prompt_sub.start.total_seconds() * 1000
            end_time = prompt_sub.end.total_seconds() * 1000
            if i == 0:
            ...
            if text_audio_dur_time > dur_time:
                tmp_audio = self.map_vocal(
                    audio=text_audio,
                    ratio=ratio,
                    dur_time=dur_time,
                    wav_name=f"map_{i}_refer.wav",
                    temp_folder=out_put_path,
                )
                tmp_audio += AudioSegment.silent(
                    dur_time - tmp_audio.duration_seconds * 1000
                )
            else:
                tmp_audio = text_audio + AudioSegment.silent(
                    dur_time - text_audio_dur_time
                )
    ...
    def map_vocal(
        ...
        writer = audiotsm.io.wav.WavWriter(
            clone_path, channels=reader.channels, samplerate=reader.samplerate
        )
...
class FishSpeech_INFER:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "prompt_audio": ("AUDIO",),
                "text": ("STRING", {"multiline": True, "default": "你好啊，世界！"}),
                "prompt_text_by_srt": (
                    "SRT",
                    {"multiline": True, "default": "a man voice"},
                ),
                "text2semantic_type": (["medium", "large"], {"default": "medium"}),
                "hf_token": ("STRING", {"default": "your token"}),
                "num_samples": ("INT", {"default": 1}),
                "max_new_tokens": ("INT", {"default": 0}),
                "top_p": ("FLOAT", {"default": 0.7}),
                "repetition_penalty": ("FLOAT", {"default": 1.5}),
    ...
    def get_tts_wav(
        ...
        npy_path = os.path.join(
            fish_tmp_out, os.path.basename(prompt_audio)[:-4] + ".npy"
        )
