...
def change_infer(
    ...
    if if_infer == True and p_infer == None:
        ...
        # 如果启用编译，设置编译环境
        if infer_compile == "Yes":
            workspace_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            env["llvm_PATH"] = os.path.join(workspace_root, "fishenv", "llvm_bin")
            env["VS140COMNTOOLS"] = os.path.join(workspace_root, "fishenv", "Tools")
            env["x64_path"] = os.path.join(workspace_root, "fishenv", "x64")
            env["TORCH_CUDA_ARCH_LIST"] = "7.5"
            env["CC"] = os.path.join(env["llvm_PATH"], "clang.exe")
            env["CXX"] = os.path.join(env["llvm_PATH"], "clang++.exe")
            env["PATH"] = f"{env['llvm_PATH']};{env['VS140COMNTOOLS']};{env['x64_path']};{env['PATH']}"
...
with gr.Blocks(
    ...
    <div style="text-align: center; margin: 10px auto;">
        <div style="display: inline-flex; justify-content: center; align-items: center; gap: 10px;">
            <a href="https://space.bilibili.com/259012968" style="display: flex; align-items: center;">
            ...
            with gr.Tab("\U0001F4D6 " + i18n("Data Preprocessing")):
                ...
                with gr.Row():
                    ...
                    initial_prompt = gr.Textbox(
                        ...
                        placeholder=i18n("This audio introduces the basic concepts and applications of artificial intelligence and machine learning."),
                        interactive=False,
                    )
                ...
                with gr.Row():
                    with gr.Column():
                        with gr.Tab(label=i18n("VQGAN Configuration")) as vqgan_page:
                            gr.HTML(i18n("You don't need to train this model!"))
